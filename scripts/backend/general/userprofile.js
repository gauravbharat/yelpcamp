...
userObj.showUserExtended = async (req, res) => {
  ...
  try {
    ...
    req.flash(
      'error',
      'Error fetching campgrounds for user profile. ' + error.message
    );
    console.log(util.getErrorStr(error));
    return res.redirect('/');
  }
...
async function updateUser(req, res, option) {
  ...
  if (option === util.utilConstants.USER_ADMIN_OPTIONS && !req.user.isAdmin) {
    req.flash(
      'error',
      'You need to be an administrator to set these user options!'
    );
    ...
    try {
      ...
      switch (option) {
        ...
        case util.utilConstants.USER_PASSWORD_CHANGE:
          ...
          let currentUser = await User.authenticate()(
            req.user.username,
          ...
          if (currentUser.error) {
            req.flash(
              'error',
              'Old password was incorrect. Try changing password again with the correct current password.'
            );
            return res.redirect('back');
          }
          ...
          // change password
          let changePwd = await user.changePassword(
            req.body.oldpassword,
        ...
        default:
          console.log(
            'No functionality option passed for function updateUser() in userprofile.js!'
          );
          return res.redirect('back');
      }
      ...
      if (option === util.utilConstants.USER_AVATAR) {
        ...
        let updatedComments = await Comment.updateMany(
          commentAuthorSearch,
          commentAuthorUpdate
        );
      }
      ...
      if (
        option === util.utilConstants.USER_ADMIN_OPTIONS &&
        notificationQuery
      ) {
  ...
  try {
    ...
    switch (foundNotification.notificationType) {
      ...
      case util.utilConstants.NOTIFY_NEW_COMMENT:
        return res.redirect(
          `/campgrounds/${foundNotification.campgroundId}/${foundNotification.commentId}`
        );
        break;
...
async function notifyAdminRequest(notificationDataObject, option) {
  ...
  try {
    ...
    if (option === util.utilConstants.CREATE) {
      errorText =
        ' Error fetching all users with admin roles in userObj.notifyAdminRequest!';
      ...
      errorText =
        ' Error deleting all admin request notifications for a user in userObj.notifyAdminRequest!';
